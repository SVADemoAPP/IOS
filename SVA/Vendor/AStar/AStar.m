...
- (NSInteger)getG:(NSInteger)col row:(NSInteger)row fid:(NSInteger)fid
{
	...
	NSInteger fx = [item id_col];
	NSInteger fy = [item id_row];
	NSInteger fg = [item id_g];
...
- (NSInteger)getH:(NSInteger)col row:(NSInteger)row
{
	...
	return labs(aimCol - col) * 10 + labs(aimRow - row) * 10;
}
...
- (void)starSearch:(NSInteger)fid withPaths:(NSMutableArray *)paths
{
    ...
    {
        NSInteger mycol = item.id_col;
        NSInteger myrow = item.id_row;
...
- (void)addToOpen:(NSInteger)col row:(NSInteger)row fid:(NSInteger)fid
{
    ...
    NSInteger g = [self getG:col row:row fid:fid];
    NSInteger h = [self getH:col row:row];
    [temp setId_g:g];
    [temp setId_h:h];
    [temp setId_f:(g + h)];
	[open addObject:temp];
...
- (BOOL)checkOpen:(NSInteger)col row:(NSInteger)row fid:(NSInteger)fid
{
	...
	for(NSUInteger i = [open count] - 1;i > 0;i --)
    {
		if([(AStarItem *)[open objectAtIndex:i] id_col] == col && [(AStarItem *)[open objectAtIndex:i] id_row] == row){
		    NSInteger tempG = [self getG:col row:row fid:fid];
...
- (NSMutableArray *)findPath:(NSInteger)curX curY:(NSInteger)curY aimX:(NSInteger)aimX aimY:(NSInteger)aimY withPath:(NSMutableArray *)paths
{
    ...
    [temp1 setId_g:0];
    NSInteger ag = [self getH:curCol row:curRow];
    [temp1 setId_h:ag];
    [temp1 setId_fid:0];
    [temp1 setId_f:ag];
	...
	while([open count] > 1) {
        ...
        NSInteger fatherid = [close count] - 1;
        
        if(labs(aimCol - [(AStarItem *)[close objectAtIndex:fatherid] id_col]) <= 3
		   && labs(aimRow - [(AStarItem *)[close objectAtIndex:fatherid] id_row]) <= 3) {
            [self getPath];
            break;
        } else {
            //搜索
